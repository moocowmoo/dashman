...
def set_vote(b, s, dir):
    ...
    b[ballot_entries[s]][u'vote'] = votes[(vote_idx[cur_vote] + dir) % len(votes)]
    return s
...
def submit_votes(win, ballot, s):
    ...
    if votes_to_send.keys():
        ...
        for vote in sorted(votes_to_send, key=lambda s: s.lower()):
            stdscr.addstr(vote + " --> " + str(votes_to_send[vote][u'vote']) + "\n")
            for mn in sorted(masternodes):
                ...
                netvote = str(node['fundtx']) + str(votes_to_send[vote][u'Hash']) + str(votes_to_send[vote][u'vote'] == 'YES' and 1 or 2) + str(random_ts)
                mnprivkey = node['mnprivkey']
                signature = dashutil.sign_vote(netvote, mnprivkey)
                command = 'dash-cli mnbudgetvoteraw ' + str(node['txid']) + ' ' + str(node['txout']) + ' ' + str(votes_to_send[vote][u'Hash']) + ' ' + str(votes_to_send[vote][u'vote']).lower() + ' ' + str(random_ts) + ' ' + signature
...
def main(screen):
	...
	with open(os.path.join(dash_conf_dir, 'masternode.conf'), 'r') as f:
		...
		for line in lines:
			conf = line.split()
			masternodes[conf[0]] = {"mnprivkey": conf[2], "fundtx": conf[3] + '-' + conf[4], "txid": conf[3], "txout": conf[4]}
